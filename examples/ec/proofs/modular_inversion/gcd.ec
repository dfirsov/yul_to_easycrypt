...
lemma simplify_divby2s t_in :
   phoare [ GCDAlgs.simplify_t : t = t_in /\ t <> 0 
                     ==> res = div_by2s t_in ] = 1%r.
proc.
while (div_by2s t = div_by2s t_in /\ t <> 0) `|t|.
progress. wp. skip. progress. smt. smt(@Int).
 case (t{hr} < 0). progress.
    have ->: `|t{hr} %/ 2| = - (t{hr} %/ 2). smt(@Int).
    have ->: `|t{hr}| = - (t{hr}). smt(@Int).
    smt(@Int).
    progress.
    have ->: `|t{hr} %/ 2| =  (t{hr} %/ 2). smt(@Int).
    have ->: `|t{hr}| =  (t{hr}). smt(@Int).
...
lemma main3_term : phoare [GCDAlgs.main3 : odd u /\ 0 < u /\ 0 < v 
                                                     ==> true ] = 1%r.
proc. unroll 3.
...
smt. smt. smt. smt. smt.
seq 1 : ((gcd u_in v_in = gcd t (if 0 < t then v else u) 
             /\ !odd t /\ odd u /\ odd v)  /\  0 < u /\ 0 < v /\ t = u - v).
wp. skip. progress;smt.
while (gcd u_in v_in =  gcd t (if 0 < t then v else u) 
              /\ !odd t /\ odd u /\ odd v /\  0 <= u /\ 0 <= v /\ t = u - v).
seq 1 : (gcd u_in v_in =gcd t (if 0 < t then v else u) 
             /\ odd t /\ odd u /\ odd v /\ t <> 0 /\ 0 <= u 
             /\ 0 <= v /\ t = div_by2s (u - v)).
wp. skip. progress;smt.
...
lemma main3_correct_and_terminating u_in v_in :
 phoare [ GCDAlgs.main3 : arg = (u_in, v_in) /\ 0 < u_in /\ 0 < v_in /\ odd u 
           ==> res = gcd u_in v_in ] = 1%r.
phoare split ! 1%r 0%r. smt().
conseq main3_term. smt(). hoare. simplify.
apply gcd_odd_alg.
qed.    


lemma main3_main4 u_in v_in :
 equiv [ GCDAlgs.main3 ~ GCDAlgs.main4 : arg{1} = (u_in, v_in) /\ 0 < u_in /\ 0 < v_in
               /\ odd u_in /\ ={arg} ==> ={res} ].
proc.
seq 2 2 : (={t,u,v}).
wp. skip. progress.
...
lemma main4_correct_and_terminating u_in v_in :
   phoare [ GCDAlgs.main4 : arg = (u_in, v_in) /\ 0 < u_in /\ 0 < v_in /\ odd u 
               ==> res = gcd u_in v_in ] = 1%r.
bypr.
progress.
